...
class StatusController {
    ...
    myStatus = async (req, res) => {
        ...
        try {
            const myStatusData = await this.statusService.myStatus(
                userId,
                isBusiness
            );
            if (!myStatusData) {
    ...
    serviceStatusUpdate = async (req, res) => {
        ...
        const existStatus = await this.statusService.existStatus(
            serviceId,
            businessId
        );
        if (existStatus.errorMessage) {
            return res
                .status(existStatus.code ? existStatus.code : 409)
                .json({ errorMessage: existStatus.errorMessage });
        }
    ...
    receiveOrder = async (req, res) => {
        ...
        const existStatus = await this.statusService.receiveOrder(
            serviceId,
            businessId,
            isBusiness
        );
