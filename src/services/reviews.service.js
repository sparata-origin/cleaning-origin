...
class ReviewService {
  ...
  writeReview = async (serviceId, businessId, customerId, content, star) => {
    try {
      ...
      if (parseInt(customerId) !== existService.customerId) {
        return {
          errorMessage: "해당 서비스 신청한 사람만 리뷰 작성 가능합니다",
        };
      }
      if (parseInt(businessId) !== existService.businessId) {
        return {
          errorMessage: "해당 서비스는 아직 업체가 정해지지 않았습니다",
        };
      }
  ...
  findAllReview = async (businessId) => {
    try {
      const allReview = await this.reviewsRepository.findAllReview(businessId);
      if (allReview.length < 1) {
        return { errorMessage: "리뷰가 존재하지 않습니다" };
      }
      const starAVG = await this.reviewsRepository.findBusinessStar(businessId);
