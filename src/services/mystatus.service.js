...
class StatusService {
    ...
    myStatus = async (userId, isBusiness) => {
        try {
            if (isBusiness) {
                const myStatusData = await this.statusRepository.buisnessStatus(
                    userId
                );
            ...
            const myStatusData = await this.statusRepository.customerStatus(
                userId
            );
    ...
    receiveOrder = async (serviceId, businessId, isBusiness) => {
        try {
            const existService = await this.statusRepository.existService(
                businessId
            );
            const existServiceList = await existService.filter(
                (data) => data.status === '청소중'
            );
            if (existServiceList.length >= 1) {
                return {
                    errorMessage: '한 번에 한 개의 의뢰만 진행할 수 있습니다.',
                };
            }
            ...
            const existStatus = await this.statusRepository.existStatus(
                serviceId
            );
